...
sequenceDiagram
    ...
    F->>L: 查询改写(小调用)
    L-->>F: "云杉ERP标准版年费价格"
    F->>V: 原问题+改写 双路向量检索(Top12)
    F->>V: 双路 BM25 关键词检索(Top12)
    V-->>F: 候选片段
    F->>F: RRF融合 + 阈值过滤(0.4) + 规则类软加成
    alt 检索为空
...
| 每日 ≤ 100 次（可配） | `services/usage.py` + `daily_usage` 表 | 原子自增（UPDATE 自带上限条件），超限 429 |
| 检索为空不编造 | `services/rag.py` 阈值过滤 | 低于 0.4 → 兜底话术（`FALLBACK_REPLY` 可配），**不调用 LLM** |
