...
| Agent 记忆系统 | L0-L3 分层（对话→原子记忆→画像）、写入去重、画像冲突检测、遗忘机制、user_id 硬隔离、问答前个性化注入 |
| 意图识别 | 产品咨询 / 售后问题 / 闲聊 / 投诉，会话记录自动标注 |
| 用户反馈 | 点赞/踩 + 选填文字 |
...
两段式检索（多路召回保覆盖率 → LLM 重排保精度）；Parent-Child 分块（180 字子块检索精准命中，完整父块进 Prompt 保上下文）；Agent 记忆系统（L0-L3 分层、去重、冲突检测、遗忘、user_id 硬隔离）；离线评估（LLM-as-judge）faithfulness 1.00。
...
$env:ADMIN_PASSWORD = "admin123"   # init_db 需要；生产环境使用强密码
...
访问 **http://localhost:5173**。管理员账号 `13800000000`，密码为初始化时 `ADMIN_PASSWORD` 设置的值。
...
```bash
cd backend
.venv/Scripts/python -m pytest tests/ -v          # 27 项单元/集成测试
.venv/Scripts/python scripts/smoke_test.py         # 端到端冒烟
.venv/Scripts/python scripts/qa_check.py           # 问答质量验证（6 题）
.venv/Scripts/python -m eval.rag_eval              # 离线评估（faithfulness）
```

测试覆盖：分块策略、认证、500 字限制、每日限额原子计数、上传格式校验、文档生命周期、越权防护、遗忘机制、管理后台权限、SSE 事件序列、空检索兜底。

## 目录结构

```
├── backend/            # FastAPI：RAG 链路、SSE、知识库、认证、限额
│   ├── app/services/   # rag / llm / embedding / vector_store / knowledge / usage / memory
│   ├── tests/          # pytest 测试套件
│   ├── scripts/        # 初始化 / 冒烟 / 质量验证 / 数据回填脚本
│   └── data/seed_docs/ # 3 篇示例文档（启动自动向量化）
├── frontend/           # React 19 + TS：流式对话 / 知识库 / 记忆 / 后台
...
- [项目说明.md](项目说明.md) — 技术选型、AI 工程问题处理、AI 工具使用体会、回答质量验证
