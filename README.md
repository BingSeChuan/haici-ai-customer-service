...
两段式检索（多路召回保覆盖率 → LLM 重排保精度）；Parent-Child 分块（180 字子块作检索单元，完整父块进 Prompt 保上下文）；Agent 记忆系统（L0-L3 分层、去重、冲突检测、遗忘、user_id 硬隔离）；离线评估（LLM-as-judge）faithfulness 1.00。
